...
function ws_parallax(k, g, a) {
	...
	var e = c("<div>").css({
		...
		height : "100%",
		overflow : "hidden"
	}).addClass("ws_effect ws_parallax").appendTo(a.parent());
	function j(l) {
	...
	this.go = function(l, r, p) {
		...
		d(
				function(v) {
					v = c.easing.swing(v);
					var x = j(p * v * m), u = j(p * (-m + v * m)), t = j(-p * m
							* b * v), w = j(p * m * b * (1 - v));
...
jQuery.extend(jQuery.easing, {
	...
	easeOutBackQ : function(e, f, a, j, i, g) {
		var h = (f /= i) * f;
		return a + j
				* (4 * h * f * h - 12 * h * h + 16 * h * f - 13 * h + 6 * f)
	},
	...
	easeOutBackQ2 : function(e, f, a, j, i, g) {
		var h = (f /= i) * f;
		return a + j
				* (1.5 * h * f * h - 5 * h * h + 10 * h * f - 12 * h + 6.5 * f)
	}
});
function ws_brick(f, s, g) {
	var h = jQuery, n = h(this), a = f.cols || 4, r = f.rows || 3, H = 2.5, c = 2, v = f.perspective || 2000, u = g
			.find(".ws_list"), G = [], b = 30, x = {}, o = h("<div>").addClass(
			"ws_effect ws_brick").css("overflow",
			f.responsive > 1 ? "hidden" : "visible"), q = f.support.transform
			&& f.support.transition && f.support.perspective, p = /Safari/
			.test(navigator.userAgent)
			&& !/Chrome/.test(navigator.userAgent), m = /Firefox/
			.test(navigator.userAgent);
	g = g.parent().append(o);
	var y = [ {
		zIndex : 0,
	...
	for (var l = 0; l < a * r; l++) {
		...
		}).appendTo(E), w = h("<div>").addClass("ws_front_image").appendTo(z), D = q ? h(
				"<div>").addClass("ws_back_image").appendTo(z)
				: 0;
		w.css({
			position : "absolute",
			width : "100%",
		...
		G[l] = {
			...
			back : D,
			wrapper : E,
			leftEdge : q ? h("<div>").addClass("ws_left_edge").css(t).appendTo(
					z) : 0,
			rightEdge : q ? h("<div>").addClass("ws_right_edge").css(t)
					.appendTo(z) : 0,
			topEdge : q ? h("<div>").addClass("ws_top_edge").css(t).appendTo(z)
					: 0,
			bottomEdge : q ? h("<div>").addClass("ws_bottom_edge").css(t)
					.appendTo(z) : 0
		}
	}
	function C(N) {
	...
	function I(J, M, j, K, L) {
		for ( var i in M) {
			M[i].topEdge.css({
				...
				height : J,
				background : j[i],
				transform : "rotateX(90deg) translate3d(0,-" + J / 2 + "px,"
						+ J / 2 + "px)"
			});
			M[i].bottomEdge.css({
				width : K,
			...
			M[i].leftEdge.css({
				...
				height : L,
				background : j[i],
				transform : "rotateY(90deg) translate3d(" + J / 2 + "px,0,-"
						+ J / 2 + "px)"
			});
			M[i].rightEdge.css({
				width : J,
		...
		var J = function() {
			...
			for ( var Q in L) {
				...
				if (R <= 0.5) {
					...
					O = h.easing.easeOutBack(1, (R - 0.5) * 2, 0, 1, 1, 1)
							.toFixed(3);
					M = h.easing.easeOutBackQ(1, (R - 0.5) * 2, 0, 1, 1, 1)
							.toFixed(3);
					N = h.easing.easeOutBackQ2(1, (R - 0.5) * 2, 0, 1, 1, 1)
							.toFixed(3);
					L[Q].back.css("backfaceVisibility", "visible")
				}
				for ( var V in L[Q].animate[R <= 0.5 ? "half" : "end"]) {
					var T = L[Q].animate[R <= 0.5 ? "begin" : "half"][V] || 0, S = L[Q].animate[R <= 0.5 ? "half"
							: "end"][V] || 0;
					if (typeof S !== "object") {
					...
					if (V === "rotateX" || V === "rotateY" || V === "rotateZ") {
						...
						if (V === "scale") {
							...
							if (V === "translate3d") {
								K += V + "("
										+ (T[0] + (S[0] - T[0]) * O).toFixed(3)
										+ "px,"
										+ (T[1] + (S[1] - T[1]) * O).toFixed(3)
										+ "px,"
										+ (T[2] + (S[2] - T[2]) * O).toFixed(3)
										+ "px) "
							} else {
								i[V] = S
							}
				...
				L[Q].wrapper.css({
					transform : "translate3d("
							+ (i.left ? i.left : 0).toFixed(3) + "px,"
							+ (i.top ? i.top : 0).toFixed(3) + "px,0)"
				});
				delete i.left;
				delete i.top;
				if (K) {
	...
	function k(aa, M, N, P) {
		var X = g.width(), W = g.height(), V = X / a, U = W / r, L = (f.duration * 0.4) > 1000 ? 1000
				: (f.duration * 0.4), K = f.duration * 0.6, Q = [ 0, 0 ];
		I(b, aa, x[M], V, U);
		o.css({
		...
		for ( var S in aa) {
			var R = aa[S], Z = S % a, Y = Math.floor(S / a), T = X * Z / a, O = W
					* Y / r;
			R.animate = {
	...
	this.go = function(X, M) {
		...
		if (q) {
			...
			var T = g.width(), W = g.height(), S = T / a, V = W / r, L = T - S
					* (a - 1), U = W - V * (r - 1);
			o.css({
				width : T,
				height : W
	...
	function F(S, J) {
		...
		try {
			R = L.getImageData(J.x ? J.x : 0, J.y ? J.y : 0, J.w ? J.w
					: S.width, J.h ? J.h : S.height)["data"]
		} catch (T) {

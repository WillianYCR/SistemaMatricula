...
@Repository
public class UsuarioAlumnoDaoImpl extends AbstractHibernateDao implements
		UsuarioAlumnoDao {
	...
	public Integer obtenerUltimoIdUsuarioAlumno() {
		abrirSesion();
		query = session
				.createSQLQuery("select max(idusuarioalumno+1) from UsuarioAlumno");
	...
	public Integer obtenerIdUsuarioPorIdAlumno(int idAlumno) {
		...
		int idUsuario = ((UsuarioAlumno) criteria.uniqueResult()).getUsuario()
				.getIdUsuario();
		cerrarSesion();
		return idUsuario;
	}
